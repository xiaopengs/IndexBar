    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                mStartXTemp = event.getX();
                return true;
            case MotionEvent.ACTION_MOVE:
                float x = event.getX();
                if (x - mStartXTemp > 0) {
                    progress = x - mStartXTemp >= TOUCH_MOVE_MAX ? 1 : (x - mStartX) / (TOUCH_MOVE_MAX);
                    setDirection(DIRECTION_RIGHT);
                    setProgress(progress);
                } else if (x - mStartXTemp < 0) {
                    progress = mStartXTemp - x >= TOUCH_MOVE_MAX ? 1 : (mStartXTemp - x) / (TOUCH_MOVE_MAX);
